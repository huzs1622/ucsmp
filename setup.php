...
$_SESSION['language'] = (isset($_GET['language'])) ? $_GET['language'] : ((isset($_SESSION['language'])) ? $_SESSION['language'] : 'zh-cn');

require_once(file_exists(LANGUAGES . $_SESSION['language'] . '.php') ? LANGUAGES . $_SESSION['language'] . '.php' : LANGUAGES . 'zh-cn.php');
...
switch($step){
	case '1':
		...
		$out .= '<p><b>' . PERFORM_PERMISSION_CHECKING . ' :</b></p>';
		...
		$out .= '<p><b>' . PERFORM_COMPABILITIES_CHECK . ' :</b></p>';
	...
	case '2':
		...
		$status = '';
		$dbHost = $form->post('dbHost', ((isset($config['dbHost'])) ? $config['dbHost'] : 'localhost'), 'strtolower');
		$dbUser = $form->post('dbUser', ((isset($config['dbUser'])) ? $config['dbUser'] : 'root'));
		$dbPass = $form->post('dbPass', ((isset($config['dbPass'])) ? $config['dbPass'] : 'root'));
		$dbName = $form->post('dbName', ((isset($config['dbName'])) ? $config['dbName'] : 'ucsmp'));
	...
	case '3':
		...
		$status = '';
		$name = $form->post('name', 'admin', 'strtolower');
		$emailAddress = $form->post('emailAddress', 'admin@admin.com', 'strtolower');
		$password = $form->post('password', 'admin888');
		...
		if($form->isPost()){
			...
			if(!$status){
				...
				$_SESSION['password'] = hashed($password);
		...
		<form action="?step=3" method="post">
			...
			<input type="text" name="password" id="password" value="' . $password . '" maxlength="100" class="text" /> <span class="red">*</span>
...
ALTER TABLE `user`
	...
	ADD INDEX `idx_date_created` (`date_created`);';
		...
		if(!defined('UPGRADE')) $db->insert('user', array('is_admin'=>1, 'is_active'=>1, 'name'=>$_SESSION['name'], 'email_address'=>$_SESSION['email_address'], 'password'=>$_SESSION['password'], 'language'=>$_SESSION['language'], 'date_created'=>date('Y-m-d H:i:s')));
...
?>';
		...
		if(defined('UPGRADE')){
			...
			<p>' . str_replace(array('%emailAddress%', '%password%'), array($_SESSION['email_address'], $_SESSION['password']), YOU_CAN_NOW_LOGIN) . '</p>

			<p><input class="button" type="button" value="' . FINISH . '" onclick="window.location.href=\'?q=log-in\';" /></p>';
			rename("setup.php", "#setup.php");
		}
	break;
